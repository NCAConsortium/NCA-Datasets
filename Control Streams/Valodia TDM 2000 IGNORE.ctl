...
$PROB RUN# SIM Valodia

;************************************************************
;****                   DATASET                          ****
;************************************************************
$INPUT C ID TIME AMT DV EVID MDV ADDL II TBW AGE DIAG
...
; RACE: 
; AGE:	Age(yr)
; SMK:	Smoking status (0 = Healthy Volunteer, 1 = Psoriasis)
...
$PK
	...
	GPVMAX= TVVMAX*((TBW/90)**THETA(12))*((AGE/40)**THETA(13))*(THETA(14)**DIAG)
	PPVVMAX= EXP(ETA(6))
	VMAX  = GPVMAX * PPVVMAX					; INDIVIDUAL VMAX
...
0.236 							; KA SC ABSORPTION RATE CONSTANT (1/HR)
5.40							; VMAX (MG/DAY)
0.02							; KM (UG/ML) 					
0.598							; TBW ON CL & Q
0.468							; AGE ON CL
...
$OMEGA
0.4789                     		; [E] BETWEEN SUBJECT VARIABILITY (CL)
0.4844                   		; [E] BETWEEN SUBJECT VARIABILITY (V1)
0.0225                   	    ; [E] BETWEEN SUBJECT VARIABILITY (Q)
0.7327                     		; [E] BETWEEN SUBJECT VARIABILITY (V2)
0.3352                  		; [E] BETWEEN SUBJECT VARIABILITY (KA)
0.0671                   	    ; [E] BETWEEN SUBJECT VARIABILITY (VMAX)					

$SIGMA
0.0141                     		; [P] PROPORTIONAL RESIDUAL ERROR
...
$TABLE                                       ; STANDARD TABLE
       ID TIME ADDL II AMT DV EVID MDV DV PRED MDV IWRE IRES
       IPRE CL V1 Q2 V2 Q3 V3 KA ETA1 ETA2 ETA3 ETA4 ETA5 ETA6 TBW AGE DIAG
